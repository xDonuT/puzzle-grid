    ...
    // ---------- match detection & shape analysis ----------
    // Both live in engine.js (pure logic, no DOM). See js/engine.js.
    ...
    async function trySwap(r1, c1, r2, c2) {
      ...
      const tmpSt = tileStatus[r1][c1];
      tileStatus[r1][c1] = tileStatus[r2][c2];
      tileStatus[r2][c2] = tmpSt;
    ...
    window.addEventListener("pointermove", onNextFrame(e => {
      ...
      const max = 34;
      const follow = 0.55;
      const clampedX = clamp(dx * follow, -max, max);
      const clampedY = clamp(dy * follow, -max, max);

      // Subtle rotation based on horizontal drag
      const tilt = clamp(dx * 0.06, -9, 9);
